...
const GatePuzzle5 = ({ onSolve, notation }: Props) => {
  const [inputA, setInputA] = useState(false);
  const [inputB, setInputB] = useState(false);

  const output = !(inputA || inputB); // NOR logic
  ...
  return (
    <div className="min-h-screen bg-black text-lime-300 font-mono flex flex-col items-center justify-center p-8 tracking-wide">
      <h1 className="text-2xl md:text-3xl font-bold mb-6 border-b border-lime-500 pb-2">
        Logic Puzzle 5: NOR Gate
      </h1>

      <div className="mb-6 text-center max-w-xl">
        <p className="mb-2">
          This gate opens only when both Input A and Input B are{" "}
          <strong>false</strong>.
        </p>
        <p>
          It uses a{" "}
          <code className="text-lime-400">NOR (Not OR)</code> logic gate —
