...
const GatePuzzle3 = ({ onSolve, notation }: Props) => {
  const [inputA, setInputA] = useState(false);
  const [inputB, setInputB] = useState(false);

  const output = (inputA || inputB) && !(inputA && inputB); // XOR logic
  ...
  return (
    <div className="min-h-screen bg-black text-lime-300 font-mono flex flex-col items-center justify-center p-8 tracking-wide">
      <h1 className="text-2xl md:text-3xl font-bold mb-6 border-b border-lime-500 pb-2">
        Logic Puzzle 3: XOR Gate
      </h1>

      <div className="mb-6 text-center max-w-xl space-y-2">
        <p>To unlock the gate, exactly one input must be <strong>TRUE</strong>.</p>
        <p>The gate uses an <code className="text-lime-400">XOR</code> (exclusive OR) logic gate.</p>
      </div>
